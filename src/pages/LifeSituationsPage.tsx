...
const situations: Situation[] = [
  ...
  { id: "id-card", title: "Potřebuji nový občanský průkaz", description: "Výměna, ztráta nebo končící platnost. Zjistíte, co přinést a kam přijít.", category: "documents", mode: "Začněte online", icon: IdCard, keywords: "občanka občanský průkaz výměna ztráta" },
  ...
  return <main className="life-page"><div className="life-page__inner">
    ...
    <section className="life-results">
      ...
      {filtered.length > 0 ? <div className="life-grid">{filtered.map((item) => {
        ...
        const path = item.id === "moving" ? "/zivotni-situace/stehuji-se" : item.id === "dog" ? "/zivotni-situace/poridil-jsem-si-psa" : item.id === "id-card" ? "/zivotni-situace/novy-obcansky-prukaz" : null;
